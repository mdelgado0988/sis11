...
function distribuyeContrato() {
  ...
  try {   
    ...
    if(!tieneFRO)
      newCessions = newCessions.filter(item => filterConditionNotIn("FRO", item));
    else
      distribuyeSegunContrato(newCessions, resultado, {
        tipoContrato: "FRO",
        usaCedant: false,
...
function distribuyeSegunContrato(newCessions, resultado, config) {
  ...
  items.forEach(ces => {
    ...
    const primaTotal = redondear(gridDataSelected.Prima ?? 0);
    const primaNoTecnica = redondear(resultado[keys.totalPrimaNT]);
    const proporcionNoTecnica = redondear(primaNoTecnica / primaTotal, 8, true);
  ...
  reaseguradoresData.forEach((r, index) => {
    ...
    const row = $(`
      <tr data-index="${index}">
        ...
        <td>
          <input type="text" value="${formatInput(r.commission,'number')}" class="number" />
        </td>
