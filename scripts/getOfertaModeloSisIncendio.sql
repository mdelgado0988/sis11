...
try {
  ...
  const vfieldsCotizacion = "id, code, [start],[end],anualPremium,tax,anualTotal,description,insuredSum,paymentMethod,periodicity,holderId,payerId,cessionBeneficiary, sellerId";
  ...
  dataCoberturas.forEach((row, index) => {
    ...
    const nuevoRegistroCoberturas = {
      ...
      DeducibleValor: formateaNumero(row.deductible ?? 0),
