() => {
    ...
    const BatchTable = ({ data, loading,searchTotal,handleTableChange, handleViewDetail, reloadData, onOpenModal }) => {
        ...
        const columns = [
            ...
            { title: 'Excluidos', dataIndex: 'skipped', width: 80, ellipsis: true, align: 'center' },
            {/* { title: 'Estado', dataIndex: 'estadoWf', width: 100, align: 'center',
                render: (text) => {
                    return (
                        <Tag color={getColorByStatus(text)} style={{ fontWeight: 500 }}>
    ...
    const ActionToolbarDetail = ({ loading, onCalculate, onRefresh, onExclude, percent}) => {      
        return (
          <Row
    ...
    const TabContent3 = ({loteId, wfId,tableData, loadDataLoteDetalle, searchTotal, handleTableChange, 
                          loading, handleRefresh, percent, startProgress, setPercent,
                          handleCalculate, handleExclude, selectedRowDetailKeys, onSelectChange}) => {       
      ...
      const handleCalculate = () => {
        try {
          ...
          exe("ExeChain",{
            ...
            setPercent(0);
            startProgress();
            notification.info({ message: "Lote de tarificación", description: resultado.msg, duration: 10 });
            ...
            if (resultado.ok){
               exe("DoBatch", {
                  ...
                  if(x.ok)
                    notification.info({ message: "Lote de tarificación", description: `Lote ${idLoteQuote} puesto en proceso`, duration: 10 });
                }); 
            }       
