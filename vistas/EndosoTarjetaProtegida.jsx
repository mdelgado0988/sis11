/**
 * @name EndosoTarjetaProtegida
 * @purpose Endoso de cambio de suma asegurada con prima fija y registro de asegurados
 *          (MSN-000030, AXX-1589). Basada en ChangeInsuredSumEndorsement (LiveView 57):
 *          mismos comandos y el mismo reaseguro. La suma se digita como movimiento
 *          (aumento/disminucion) o como suma final y se recalcula la otra. La prima la
 *          digita el usuario, es obligatoria y viaja en jAdditional (endorsementSubtype
 *          ENDOSOTARJETAPROTEGIDA + fixedPremium): la cotizacion nativa (ChangePolicyCapital
 *          -> cadena de tarifa del producto) la devuelve distribuida por cobertura segun su
 *          proporcion. La prima se digita como movimiento firmado y se guarda en
 *          jAdditional junto con la prima final calculada. Los asegurados del endoso
 *          se guardan en jAdditional.insureds.
 */
() => {
  const Tabs = A.Tabs;
  const Card = A.Card;
  ...
  const [sumMovement, setSumMovement] = useState(null);
  const [fixedPremium, setFixedPremium] = useState(null);
  const [insureds, setInsureds] = useState([]);
  const [contactOptions, setContactOptions] = useState([]);
  const [contactSearching, setContactSearching] = useState(false);
  const [selectedContact, setSelectedContact] = useState(null);
  const [editingInsuredKey, setEditingInsuredKey] = useState(null);
  const [contactSearch] = useState({ timer: null, version: 0 });
  ...
  const [payPlanPreview, setPayPlanPreview] = useState({ old: [], next: [], loading: false, error: null });
  ...
  const policyStartValue = function (source) {
    return source && (source.start || source.activeDate || source.effectiveDate);
  };
  const effectiveDateUtc = function (value) {
    const selected = moment.utc(value);
    const policyStart = moment.utc(policyStartValue(policy));
    if (!selected.isValid()) return '';
    if (policyStart.isValid() && selected.format('YYYY-MM-DD') === policyStart.format('YYYY-MM-DD')) {
      return policyStart.format('YYYY-MM-DD') + 'T23:00:00';
    }
    return selected.clone().startOf('day').add(12, 'hours').format('YYYY-MM-DDTHH:mm:ss[Z]');
  };
  ...
  function closeEnough(left, right) {
    const leftCents = Math.round((Number(left || 0) + Number.EPSILON) * 100);
    const rightCents = Math.round((Number(right || 0) + Number.EPSILON) * 100);
    return Math.abs(leftCents - rightCents) <= 1;
  }
    ...
    exe('RepoLifePolicy', { operation: 'GET', filter: 'id=' + id, include: ['Coverages', 'Product'], size: 1 })
      .then(function (r) {
        ...
        return Promise.all([
          ...
          setSumMovement(0);
          setFixedPremium(null);
          setInsureds([]);
          setSelectedContact(null);
          setEditingInsuredKey(null);
          const policyStart = policyStartValue(p);
          const todayUtc = moment.utc().startOf('day');
          const initialDate = policyStart && moment.utc(policyStart).isValid()
            && moment.utc(policyStart).startOf('day').isAfter(todayUtc)
            ? moment.utc(policyStart)
            : moment.utc();
  ...
  function buscar() {
    ...
    invalidate(); setPolicy(null); setNewInsuredSum(null); setSumMovement(null); setEffectiveDate(null);
  ...
  function invalidate() {
    ...
    setPayPlanPreview({ old: [], next: [], loading: false, error: null });
  ...
  // ------------------------------------------------------------- suma y prima
  // La suma se puede digitar como movimiento o como suma final; cada campo recalcula el otro.
  function onSumMovementChange(v) {
    const movement = v === null || v === undefined || v === '' ? null : Number(v);
    setSumMovement(movement);
    setNewInsuredSum(movement === null || !policy ? null : money(Number(policy.insuredSum || 0) + movement));
    invalidate();
  }

  function onFinalSumChange(v) {
    const finalSum = v === null || v === undefined || v === '' ? null : Number(v);
    setNewInsuredSum(finalSum);
    setSumMovement(finalSum === null || !policy ? null : money(finalSum - Number(policy.insuredSum || 0)));
    invalidate();
  }

  function onFixedPremiumChange(v) {
    setFixedPremium(v === null || v === undefined || v === '' ? null : Number(v));
    invalidate();
  }

  function currentPolicyPremium() {
    if (!policy) return 0;
    const value = policy.coverages !== undefined
      ? policy.coverages
      : (policy.anualPremium !== undefined ? policy.anualPremium : policy.annualPremium);
    return money(value);
  }

  // El comando recibe fixedPremium como prima final, pero la vista captura la
  // variacion. Se guardan ambos valores para mantener trazabilidad y validar signos.
  function fixedPremiumAdditional(finalPremium, premiumMovement, capitalMovement) {
    return {
      endorsementType: 'CHANGE_PROTECTED_CARD',
      endorsementSubtype: 'ENDOSOTARJETAPROTEGIDA',
      fixedPremium: money(finalPremium),
      premiumMovement: money(premiumMovement),
      sumMovement: money(capitalMovement)
    };
  }

  // ------------------------------------------------------------- asegurados del endoso
  function contactFullName(contact) {
    return contact.isPerson
      ...
      : txt(contact.surname2 || contact.name);
  }

  function contactIdentification(contact) {
    return txt(contact.isPerson ? (contact.cnp || contact.nif) : (contact.nif || contact.cnp));
  }

  function contactToInsured(contact) {
    return {
      key: String(contact.id),
      contactId: Number(contact.id),
      name: contactFullName(contact),
      identification: contactIdentification(contact),
      cobis: txt(contact.nationalId),
      registrationDate: moment().format('YYYY-MM-DD')
    };
  }

  // Un solo autocomplete: nombre (name + surname1 persona, surname2 juridico), identificacion
  // (cnp/nif) o numero COBIS (nationalId).
  function searchContacts(value) {
    const text = txt(value).replace(/;/g, ' ').replace(/-{2,}/g, '-').trim();
    if (contactSearch.timer) clearTimeout(contactSearch.timer);
    if (text.length < 3) { setContactOptions([]); setContactSearching(false); return; }
    const version = ++contactSearch.version;
    setContactSearching(true);
    contactSearch.timer = setTimeout(function () {
      const like = "N'%" + text.replace(/'/g, "''") + "%'";
      // cnp va al final: en este ambiente una clausula de cnp que no es la ultima rompe el filtro.
      const filter = '((isPerson=1 AND (name LIKE ' + like + ' OR surname1 LIKE ' + like
        + " OR (ISNULL(name,'') + ' ' + ISNULL(surname1,'')) LIKE " + like + '))'
        + " OR (isPerson=0 AND ISNULL(NULLIF(surname2,''),name) LIKE " + like + ')'
        + ' OR nationalId LIKE ' + like + ' OR nif LIKE ' + like + ' OR cnp LIKE ' + like + ')';
      exe('GetContacts', { filter: filter, size: 20 })
        .then(function (r) {
          if (version !== contactSearch.version) return;
          setContactSearching(false);
          if (!r || !r.ok) { setContactOptions([]); setError(String((r && r.msg) || t('No se pudo buscar el contacto'))); return; }
          setContactOptions((r.outData || []).map(function (contact) {
            return { value: String(contact.id), contact: contact };
          }));
        })
        .catch(function (e) {
          if (version !== contactSearch.version) return;
  ...
  function onSelectContact(value) {
    if (!value) { setSelectedContact(null); return; }
    const option = contactOptions.find(function (item) { return item.value === String(value); });
    setSelectedContact(option ? option.contact : null);
  }

  function guardarAsegurado() {
    if (!selectedContact) { setError(t('Busque y seleccione el contacto del asegurado')); return; }
    const row = contactToInsured(selectedContact);
    const duplicated = insureds.some(function (item) {
      return item.key === row.key && item.key !== editingInsuredKey;
    });
    if (duplicated) { setError(t('El asegurado ya está registrado en este endoso')); return; }
    setError(null);
    if (editingInsuredKey) {
      setInsureds(insureds.map(function (item) { return item.key === editingInsuredKey ? row : item; }));
    } else {
      setInsureds(insureds.concat([row]));
    }
    setSelectedContact(null);
    setEditingInsuredKey(null);
    setContactOptions([]);
  ...
  function editarAsegurado(row) {
    setEditingInsuredKey(row.key);
    const contact = { id: row.contactId, isPerson: true, name: row.name, cnp: row.identification, nationalId: row.cobis };
    setContactOptions([{ value: String(row.contactId), contact: contact }]);
    setSelectedContact(contact);
  }

  function cancelarEdicionAsegurado() {
    setEditingInsuredKey(null);
    setSelectedContact(null);
    setContactOptions([]);
  }

  function eliminarAsegurado(row) {
    setInsureds(insureds.filter(function (item) { return item.key !== row.key; }));
    if (editingInsuredKey === row.key) cancelarEdicionAsegurado();
  }

  // ------------------------------------------------------------- pestania 1
  async function loadPayPlanPreview(calculation) {
    if (!calculation) {
      setPayPlanPreview({ old: [], next: [], loading: false, error: null });
      return;
    }
    setPayPlanPreview({ old: [], next: [], loading: true, error: null });
    try {
      let nextRows = calculation.jNewPayPlan;
      if (nextRows === undefined) nextRows = calculation.quote && calculation.quote.jNewPayPlan;
      if (typeof nextRows === 'string') {
        try { nextRows = JSON.parse(nextRows); } catch (parseError) { nextRows = []; }
      }
      if (!Array.isArray(nextRows)) nextRows = [];

      // Only use the current plan as an in-memory fallback when the endorsement
      // did not return any jNewPayPlan rows. Nothing is persisted here.
      if (!nextRows.length) {
        ...
        if (!response || !response.ok) throw new Error(response && response.msg ? response.msg : t('No se pudo consultar el plan de pagos actual'));
        const currentRows = Array.isArray(response.outData) ? response.outData : [];
        if (!currentRows.length) throw new Error(t('El endoso no contiene jNewPayPlan y no hay cuotas vigentes para previsualizar'));

        const targetTotal = Number(calculation.billing && calculation.billing.total && calculation.billing.total.after);
        const total = Number.isFinite(targetTotal) ? targetTotal : Number(calculation.quote && calculation.quote.Bill && (calculation.quote.Bill.annualTotal || calculation.quote.Bill.anualTotal));
        if (!Number.isFinite(total)) throw new Error(t('No se pudo determinar el total de facturación del endoso'));

        const rows = currentRows.map(function (row, index) {
          const amount = Number(row.minimum !== undefined ? row.minimum : row.expected) || 0;
          const paid = Number(row.payed !== undefined ? row.payed : row.paid) || 0;
        ...
        const fixedTotal = rows.reduce(function (sum, row) {
          return sum + (row.fullyPaid ? row.amount : 0);
        }, 0);
        const pendingRows = rows.filter(function (row) { return !row.fullyPaid; });
        const pendingTotal = Math.max(0, total - fixedTotal);
        let allocated = 0;
        nextRows = rows.map(function (row) {
          let amount = row.fullyPaid ? row.amount : 0;
          if (!row.fullyPaid && pendingRows.length) {
            const pendingIndex = pendingRows.indexOf(row);
            amount = pendingIndex === pendingRows.length - 1
              ? money(pendingTotal - allocated)
              : money(pendingTotal / pendingRows.length);
            allocated = money(allocated + amount);
          }
          const source = row.source || {};
          return Object.assign({}, source, {
            id: source.id || 0,
            numberInYear: source.numberInYear || source.number || row.index + 1,
            minimum: amount,
            expected: amount,
            payed: row.paid,
            dueDate: source.dueDate || source.normalDueDate || source.coveredUntil,
            normalDueDate: source.normalDueDate || source.dueDate || source.coveredUntil
          });
        });
      }
      setPayPlanPreview({ old: [], next: nextRows, loading: false, error: null });
    } catch (error) {
      setPayPlanPreview({ old: [], next: [], loading: false, error: String(error && error.message ? error.message : error) });
    }
  }

  const payPlanPreviewRows = payPlanPreview.next.map(function (nextInstallment, index) {
    const installment = nextInstallment || {};
    return {
      key: String(installment.id || installment.numberInYear || installment.number || index),
      number: installment.numberInYear || installment.number || index + 1,
      amount: installment.minimum !== undefined ? installment.minimum : installment.expected,
      paid: installment.payed !== undefined ? installment.payed : installment.paid,
      dueDate: installment.dueDate || installment.normalDueDate || installment.coveredUntil
    };
  });
  const payPlanAmountCell = function (value, alwaysRed) {
    const amount = Number(value || 0);
    return <span className={alwaysRed ? 'axx-monto-neg' : signo(amount)}>{fmt(amount)}</span>;
  };
  const payPlanTotalAmount = payPlanPreviewRows.reduce(function (sum, row) { return sum + (Number(row.amount) || 0); }, 0);
  const payPlanTotalPaid = payPlanPreviewRows.reduce(function (sum, row) { return sum + (Number(row.paid) || 0); }, 0);
  const payPlanPreviewColumns = [
    { title: t('Cuota id.'), dataIndex: 'number', key: 'number', width: 120, align: 'center' },
    { title: t('Importe de la cuota'), dataIndex: 'amount', key: 'amount', width: 150, align: 'right', render: function (value) { return payPlanAmountCell(value, false); } },
    { title: t('Pagado'), dataIndex: 'paid', key: 'paid', width: 130, align: 'right', render: function (value) { return payPlanAmountCell(value, true); } },
    { title: t('Fecha de vencimiento'), dataIndex: 'dueDate', key: 'dueDate', width: 190, align: 'center', render: function (value) { return value ? <span style={{ color: '#1677ff' }}>{day10(value)}</span> : '—'; } }
  ];
  ...
  function calcular() {
    ...
    const premiumMovement = money(fixedPremium);
    const currentPremium = currentPolicyPremium();
    const premiumValue = money(currentPremium + premiumMovement);
    const capitalMovement = money(finalSum - Number(policy.insuredSum || 0));
    if (sumMovement === null || !Number.isFinite(capitalMovement)) {
      setError(t('Indique un movimiento de suma asegurada válido')); return;
    }
    if (capitalMovement > 0 && premiumMovement < 0) {
      setError(t('Para un aumento de suma, la prima debe ser positiva o cero')); return;
    }
    if (capitalMovement < 0 && premiumMovement > 0) {
      setError(t('Para una disminución de suma, la prima debe ser negativa o cero')); return;
    }
    if (premiumValue < 0) {
      setError(t('La prima final no puede ser negativa')); return;
    }
    ...
    const quotePayload = {
      ...
      newCapital: finalSum,
      effectiveDate: effectiveDateUtc(effectiveDate),
      ...
      jAdditional: JSON.stringify(fixedPremiumAdditional(premiumValue, premiumMovement, capitalMovement))
    };
    exe('ChangePolicyCapital', quotePayload)
      ...
      .then(function (adjustedResult) {
        ...
        let endorsementPayPlan = quote.jNewPayPlan;
        if (typeof endorsementPayPlan === 'string') {
          try { endorsementPayPlan = JSON.parse(endorsementPayPlan); } catch (parseError) { endorsementPayPlan = []; }
        }
        if (!Array.isArray(endorsementPayPlan)) endorsementPayPlan = [];
        const quotedPremium = money(quote.Bill && quote.Bill.coverages);
        if (!closeEnough(quotedPremium, premiumValue)) {
          setLoading(false);
          setError(t('La cotización no aplicó la prima digitada') + ' (' + fmt(quotedPremium) + ' / ' + fmt(premiumValue) + '). '
            + t('El producto de esta póliza no tarifica con una cadena que admita prima fija.'));
          return null;
        }
        ...
        const calculation = {
          quote: quote,
          jNewPayPlan: endorsementPayPlan,
          rows: rows,
        ...
        premium: { before: beforePremium, calculated: money(afterPremium - beforePremium), after: afterPremium },
            adjustments: {
        ...
        setCalc(calculation);
        loadPayPlanPreview(calculation);
  ...
  async function ejecutar() {
    ...
    if (!insureds.length) { setModal(false); setError(t('Registre al menos un asegurado del endoso')); return; }
    ...
    const payload = Object.assign({}, calc.quote || {}, {
      ...
      jAdditional: JSON.stringify({
        endorsementType: 'CHANGE_PROTECTED_CARD',
        endorsementSubtype: 'ENDOSOTARJETAPROTEGIDA',
        view: 'EndosoTarjetaProtegida',
        fixedPremium: money(calc.billing.premium.after),
        premiumMovement: money(calc.billing.premium.after - calc.billing.premium.before),
        sumMovement: money(Number(newInsuredSum || 0) - Number(policy.insuredSum || 0)),
        insureds: insureds.map(function (row) {
          return { contactId: row.contactId, name: row.name, identification: row.identification, cobis: row.cobis, registrationDate: row.registrationDate };
        }),
  ...
  const colsAsegurados = [
    { title: t('Nombre del asegurado'), dataIndex: 'name' },
    { title: t('Identificación'), dataIndex: 'identification', width: 160 },
    { title: t('Número COBIS'), dataIndex: 'cobis', width: 140 },
    { title: t('Fecha de registro'), dataIndex: 'registrationDate', width: 130 },
    { title: t('Acciones'), dataIndex: 'key', width: 150, render: function (v, row) {
      return <span>
        <Button size="small" type="link" disabled={running} onClick={function () { editarAsegurado(row); }}>{t('Editar')}</Button>
        <Button size="small" type="link" danger disabled={running} onClick={function () { eliminarAsegurado(row); }}>{t('Eliminar')}</Button>
      </span>;
    } }
  ];
...
.axx299 .axx-asegurados { margin:4px 0; padding:4px; border:1px solid #d9e2ec; background:#f7f9fb; }
.axx299 .axx-asegurados-titulo { font-weight:600; color:#334155; padding:2px 4px; }
...
`;
  ...
  const puedeEjecutar = !!(calc && calc.rows && calc.rows.length && !running && reinsuranceConfirmed && insureds.length);
  ...
  return (
    <DefaultPage title={t('Endoso Tarjeta Protegida')} subTitle={policy ? policy.code : ''}>
      <div className="axx299">
        ...
        <Tabs className="axx-tabs" activeKey={tab} onChange={setTab} type="card"
          items={[
                ...
                <div className="axx-panel">
                  <Card bordered={false}>
                    <div className="axx-filtros">
                      ...
                      <div className="axx-campo">
                        <label>{t('Suma a aumentar (+) o disminuir (-)')}</label>
                        <InputNumber id="numMovimientoSuma" step={1000} style={{ width: 180 }} value={sumMovement}
                          disabled={!policy}
                      ...
                      <div className="axx-campo">
                        <label>{t('Movimiento de prima (+/-)')} *</label>
                        <InputNumber id="numPrima" step={1} precision={2} style={{ width: 150 }} value={fixedPremium}
                          status={policy && (fixedPremium === null || fixedPremium === undefined || !Number.isFinite(Number(fixedPremium))) ? 'error' : undefined}
                          disabled={!policy}
                          formatter={function (value) { return String(value === undefined ? '' : value).replace(/\B(?=(\d{3})+(?!\d))/g, ','); }}
                          parser={function (value) { return String(value || '').replace(/,/g, ''); }}
                          onChange={onFixedPremiumChange} />
                        {policy ? <div style={{ color: '#5a6572', fontSize: 12, marginTop: 3 }}>
                          {t('Prima final estimada')}: {fmt(currentPolicyPremium() + Number(fixedPremium || 0))}
                        </div> : null}
                      </div>
                      <Button id="btnCalcular" type="primary" loading={loading}
                        disabled={!policy || !newInsuredSum || !effectiveDate || fixedPremium === null || fixedPremium === undefined || !Number.isFinite(Number(fixedPremium))} onClick={calcular}>{t('Calcular endoso')}</Button>
                    </div>
                    {policy && sumMovement ? (
                      <div style={{ padding: '0 4px 4px 4px', color: '#5a6572', fontSize: 12 }}>
                        {Number(sumMovement) > 0 ? t('Aumento de suma asegurada') : t('Disminución de suma asegurada')}: {conSigno(sumMovement)}
                        {' | '}{t('Suma final')}: {fmt(newInsuredSum)}
                      </div>
                    ) : null}

                    <div className="axx-asegurados">
                      <div className="axx-asegurados-titulo">{t('Asegurados del endoso')} ({insureds.length})</div>
                      <div className="axx-filtros">
                        <div className="axx-campo">
                          <label>{editingInsuredKey ? t('Editar asegurado') : t('Buscar asegurado')}</label>
                          <Select id="selAsegurado" showSearch allowClear filterOption={false} style={{ width: 420 }}
                            placeholder={t('Nombre, identificación o número COBIS')} disabled={!policy || running}
                            value={selectedContact ? String(selectedContact.id) : undefined}
                            notFoundContent={contactSearching ? <Spin size="small" /> : null}
                            onSearch={searchContacts} onChange={onSelectContact}>
                            {contactOptions.map(function (option) {
                              const c = option.contact;
                              const ident = contactIdentification(c);
                              return <Select.Option key={option.value} value={option.value}>
                                {contactFullName(c) + (ident ? ' — ' + ident : '') + (txt(c.nationalId) ? ' — COBIS ' + txt(c.nationalId) : '')}
                              </Select.Option>;
                            })}
                          </Select>
                        </div>
                        <div className="axx-campo">
                          ...
                          <Input id="txtAseguradoIdentificacion" readOnly style={{ width: 150 }} value={selectedContact ? contactIdentification(selectedContact) : ''} />
                        </div>
                        <div className="axx-campo">
                          <label>{t('Número COBIS')}</label>
                          <Input id="txtAseguradoCobis" readOnly style={{ width: 130 }} value={selectedContact ? txt(selectedContact.nationalId) : ''} />
                        </div>
                        <div className="axx-campo">
                          <label>{t('Fecha de registro')}</label>
                          <Input id="txtAseguradoFecha" readOnly style={{ width: 110 }} value={selectedContact ? moment().format('YYYY-MM-DD') : ''} />
                        </div>
                        <Button id="btnAgregarAsegurado" type="primary" disabled={!selectedContact || running} onClick={guardarAsegurado}>
                          {editingInsuredKey ? t('Guardar cambios') : t('Agregar')}
                        </Button>
                        {editingInsuredKey ? <Button className="axx-btn-sec" onClick={cancelarEdicionAsegurado}>{t('Cancelar')}</Button> : null}
                      </div>
                      <Table className="axx-grilla-asegurados" size="small" pagination={false} rowKey="key"
                        dataSource={insureds} columns={colsAsegurados}
                        locale={{ emptyText: t('Sin asegurados registrados: agregue al menos uno para ejecutar el endoso') }} />
                    </div>
                    <div style={{ height: 8 }} />
                    ...
                    <Spin spinning={loading}>
                      ...
                      ) : <Empty description={t('Indique la suma (movimiento o suma final), la prima y pulse Calcular endoso')} />}
                    </Spin>
                  </Card>
                </div>
              )
            ...
            {
              key: 'installments', label: t('Vista previa de cuotas'), children: (
                <div className="axx-panel">
                  <Card bordered={false}>
                    {!calc ? <Empty description={t('Calcule el endoso para visualizar las cuotas')} /> : null}
                    {calc && payPlanPreview.error ? <Alert type="error" showIcon message={payPlanPreview.error} /> : null}
                    {calc && !payPlanPreview.error ? (
                      <Table className="axx-grilla" size="small" bordered pagination={false} rowKey="key"
                        loading={payPlanPreview.loading}
                        dataSource={payPlanPreviewRows}
                        columns={payPlanPreviewColumns}
                        summary={function () {
                          return <Table.Summary>
                            <Table.Summary.Row className="axx-rea-total-row">
                              <Table.Summary.Cell index={0}><b>{t('Totales')}</b></Table.Summary.Cell>
                              <Table.Summary.Cell index={1} align="right">{payPlanAmountCell(payPlanTotalAmount, false)}</Table.Summary.Cell>
                              <Table.Summary.Cell index={2} align="right">{payPlanAmountCell(payPlanTotalPaid, true)}</Table.Summary.Cell>
                              <Table.Summary.Cell index={3}></Table.Summary.Cell>
                            </Table.Summary.Row>
                          </Table.Summary>;
                        }}
                        locale={{ emptyText: t('No hay cuotas para mostrar') }}
                        scroll={{ x: 780 }} />
                    ) : null}
                  </Card>
                </div>
          ...
          <div>
            {calc ? (
              <div style={{ marginBottom: 8 }}>
                ...
                {t('Total de la poliza')} {fmt(calc.billing.total.before)} → <b>{fmt(calc.billing.total.after)}</b><br />
                {t('Movimiento de prima digitado')}: <b>{conSigno(fixedPremium)}</b> {calc.billing.currency}<br />
                {t('Asegurados registrados')}: <b>{insureds.length}</b>
              </div>
            ) : null}
