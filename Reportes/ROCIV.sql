...
DECLARE @TransferId INT = 11011;

SELECT
    t.id AS TransferId,
    COALESCE(formData.ClientePA, formData.Pagador, formData.Cliente, formData.Contacto, '') AS Recibido,
    COALESCE(formData.ClientePA, formData.Pagador, formData.Cliente, formData.Contacto, '') AS PorCuenta,
    FORMAT(t.amount, 'N2', 'en-US') AS Total,
    CONCAT('CONVERTIR_A_LETRAS(', CONVERT(VARCHAR(50), t.amount), ')') AS Enletras,
    COALESCE(u.nombre, tw.[user], '') AS Cajero,
