...
try {
  ...
  const cuotasPoliza = getPolicyInstallments(change.lifePolicyId ?? 0);    
  //return cuotasPoliza

  //Si es solo informativo mantengo las cuotas de la póliza
  if(change?.informative){
    normalizeInformativeBill(change);
    const resultado = retornaCuotasPoliza(cuotasPoliza);
    return { ok: true, msg: "Informativo", cuotas: JSON.stringify(resultado), cuotasDet: resultado, Bill: change.Bill, BillDiff: change.BillDiff ?? null };
  }

  const cuotasPago = getPayedInstallment(change.lifePolicyId);  
  const cuotas = safeJsonArray(change?.jNewPayPlan);
  ...
  return { ok: true, 
            msg: "Distribuido", 
            cuotas: JSON.stringify(cuotasDistribuye), 
            cuotasDet: cuotasDistribuye,
            calculado: totalCalculado, 
            totalEndoso: totalBill, 
            cuotasOld: cuotas,
            Bill: change.Bill,
            BillDiff: change.BillDiff ?? null
          };
...
function retornaCuotasPoliza(cuotasPoliza) {
  const resultado = cuotasPoliza.map(x => ({
      id: x.id,
      lifePolicyId: x.lifePolicyId,
      concept: x.concept,
      expected: x.expected,
...
function getPolicyInstallments(policyId) {
  doCmd({cmd: "LoadEntities", data: { entity: "PayPlan", filter: `lifepolicyId = ${policyId} AND cancellationDate IS NULL` }});
...
function normalizeInformativeBill(change) {
  if (!change?.Bill) return;
  ...
  bill.jTaxes = zeroJsonAmounts(bill.jTaxes);
  bill.jFees = zeroJsonAmounts(bill.jFees);

  if (change.BillDiff) {
    normalizeBillAmounts(change.BillDiff);
  }
}

function zeroJsonAmounts(rawJson) {
  if (!rawJson) return rawJson;

  try {
    const value = typeof rawJson === "string" ? JSON.parse(rawJson) : rawJson;

    if (!Array.isArray(value)) return rawJson;
    ...
    return typeof rawJson === "string" ? JSON.stringify(sanitized) : sanitized;
  } catch (error) {
    return rawJson;
  }
}

function normalizeBillAmounts(bill) {
  if (!bill) return;
...
function safeJsonArray(rawJson) {
  if (!rawJson) return [];
