//block
/***********************************************************************************************\
|* @Name: getOfertaModeloSisIncendio                                                           *|
...
try {
  ...
  const vfieldsCotizacion = "id, code, lob, productCode, [start],[end],anualPremium,tax,anualTotal,description,insuredSum,paymentMethod,periodicity,holderId,payerId,cessionBeneficiary, sellerId";
  ...
  const cramo = dataCotizacion.lob;
  const cplan = dataCotizacion.productCode;
  ...
  doCmd({cmd:"LoadEntities",data:{entity:"lifecoverage",operatiion:"GET",filter:"LifePolicyId = "+_id,fields:vfieldsCoberturas}});
  const dataCoberturas = LoadEntities.outData;

  dataCoberturas.sort((a, b) => Number(a.number) - Number(b.number));
  ...
  doCmd({cmd:"RepoInsuredObject",data:{filter:"LifePolicyId = "+_id,"operation":"GET"}});
  ...
  if (formDataArray !== null){
    ...
    Riesgo.NombreBarriada =  objetoCamposValores.txtBarriadas ?? "";
    Riesgo.NombreEdificio =  objetoCamposValores.txtEdificios ?? "";
  };
  
  //Obteniendo los nombres de los códigos
  //País
  doCmd({cmd:"RepoCountryCatalog",data:{operation:"GET", filter: "code="+ Riesgo.CodigoPais}});
  const dataPais = RepoCountryCatalog?.outData?.[0] || 0;
  ...
  const dataProvincia = RepoStateCatalog?.outData?.[0] || 0;
  ...
  const dataDistrito = RepoCityCatalog?.outData?.[0] || 0;
  ...
  const dataSector = RepoSectorCatalog?.outData?.[0] || 0;
  ...
  Cotizacion.code = dataCotizacion.code ?? "0";
  ...
  dataCoberturas.forEach((row, index) => {
    ...
    DescripcionValor = getDescripcionCober(row.code, TarifasCatalog, cplan, SumaCob256);
...
function getDescripcionCober(ccober, TarifasCatalog, cplan, SumaCob256) {
  let rowdescripcion;
  let returnDescripcion="";
      
  rowdescripcion = TarifasCatalog.find(x => x.cramo == 1 && x.codigoplan == cplan && x.ccobertura == Number(ccober));
  if (rowdescripcion) {
    returnDescripcion = returnDescripcion = simulateTarifa(rowdescripcion.etiqueta, ccober, SumaCob256);
  }
