...
const cancellationChange = isCancellationChange(changeId);

// Adjust cancellation reinsurance before validating the manual distribution.
// The regular flow can continue using the corrected values.
if (cancellationChange) {
  adjustCancellationReinsurance(policyId, changeId);
}
...
function isCancellationChange(changeId) {
  doCmd({
    cmd: "LoadEntity",
    data: {
      entity: "Change",
  ...
  const discriminator = String(LoadEntity.outData?.Discriminator || "").trim();
  return discriminator === "CancellationChange";
}

function adjustCancellationReinsurance(policyId, changeId) {
  const targetChangeId = Number(changeId);
  if (!Number.isInteger(targetChangeId) || targetChangeId <= 0 || Number(policyId) <= 0) {
    return;
  }
  ...
  const detail = safeJson(LoadEntity.outData?.jDetail, {});
  const hasCoveragesDifference = detail.coveragesDif !== null
    && detail.coveragesDif !== undefined
    && detail.coveragesDif !== ""
    && Number.isFinite(Number(detail.coveragesDif));
  if (!hasCoveragesDifference) {
    throw new Error("El detalle del endoso no contiene coveragesDif para calcular la cancelación de reaseguro");
  }
  const targetPremium = toNumber(detail.coveragesDif);

  doCmd({
    cmd: "LoadEntities",
    data: {
      entity: "Cession",
      fields: "id,coverageId,lineId,changeId,premium,sumInsuredCedant,sumInsuredRe,proportionCed,proportionRe,premiumCedant,premiumRe,comissionCedant,tax",
      // Include both unlinked rows from the first run and rows already linked
      // to this endorsement so retries can deterministically recalculate them.
      filter: `lifePolicyId = ${policyId} AND premiumType = 'CANCELLATION' AND (changeId IS NULL OR changeId = ${targetChangeId})`,
      noTracking: true
    }
  });

  const cancellationCessions = Array.isArray(LoadEntities.outData)
    ? LoadEntities.outData
    : [];
  if (cancellationCessions.length === 0) {
    return;
  }

  // Keep the latest row for each coverage and reinsurance line.
  const latestByCoverageLine = cancellationCessions.reduce((result, cession) => {
    const coverageId = String(cession && cession.coverageId || "");
    const lineId = String(cession && cession.lineId || "");
    const key = `${coverageId}|${lineId}`;
    const current = result[key];

    if (!current || Number(cession && cession.id || 0) > Number(current.id || 0)) {
      result[key] = cession;
    }
  ...
  const cessions = Object.values(latestByCoverageLine);
  const cessionIds = cessions
  ...
  const statements = cessions.map((cession, index) => {
    ...
    const premium = index === cessions.length - 1
      ? Math.sign(targetPremium) * (targetAbsPremium - allocatedPremium)
      : Math.sign(targetPremium) * Math.round(targetAbsPremium * weight * 100) / 100;
    allocatedPremium += Math.abs(premium);

    const cedantInsured = Math.abs(toNumber(cession.sumInsuredCedant));
    const reInsured = Math.abs(toNumber(cession.sumInsuredRe));
    const insuredSplit = cedantInsured + reInsured;
    const currentCedant = Math.abs(toNumber(cession.premiumCedant));
    const currentRe = Math.abs(toNumber(cession.premiumRe));
    const currentPremiumSplit = currentCedant + currentRe;
    const configuredCedant = Math.max(toNumber(cession.proportionCed), 0);
    const configuredRe = Math.max(toNumber(cession.proportionRe), 0);
    const configuredSplit = configuredCedant + configuredRe;
    const cedantRatio = insuredSplit > 0
      ? cedantInsured / insuredSplit
      : configuredSplit > 0
      ? configuredCedant / configuredSplit
      : currentPremiumSplit > 0
      ? currentCedant / currentPremiumSplit
      : 0;
    const reRatio = insuredSplit > 0
      ? reInsured / insuredSplit
      : configuredSplit > 0
      ? configuredRe / configuredSplit
      : currentPremiumSplit > 0
      ? currentRe / currentPremiumSplit
      : 0;
    const commissionRatio = toNumber(cession.premiumRe) !== 0
      ? toNumber(cession.comissionCedant) / toNumber(cession.premiumRe)
      : 0;
    const taxRatio = toNumber(cession.premiumRe) !== 0
      ? toNumber(cession.tax) / toNumber(cession.premiumRe)
      : 0;
    const premiumCedant = Math.round(premium * cedantRatio * 100) / 100;
    const premiumRe = Math.round(premium * reRatio * 100) / 100;
    const commission = Math.round(premiumRe * commissionRatio * 100) / 100;
...
SET changeId = ${targetChangeId},
    ...
    comissionCedant = ${commission.toFixed(2)},
    tax = ${tax.toFixed(2)}
WHERE id = ${id}
