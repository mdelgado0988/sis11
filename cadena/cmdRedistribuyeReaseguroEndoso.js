...
function adjustCancellationReinsurance(policyId, changeId) {
  ...
  const oldCoverages = Math.abs(toNumber(detail.oldCoverages));
  const coveragesDifference = Math.abs(toNumber(detail.coveragesDif));
  if (oldCoverages <= 0 || coveragesDifference <= 0) {
    return;
  }
  ...
  doCmd({
    ...
    data: {
      entity: "Cession",
      fields: "id,coverageId,lineId,premium,premiumCedant,premiumRe",
      // Cancellation rows may be overwritten, so use the cancellation rows
      // generated by this endorsement instead of filtering overwritten.
      filter: `lifePolicyId = ${policyId} AND premiumType = 'CANCELLATION'`,
      noTracking: true
    }
  });
  ...
  // Keep only the latest row for each coverage and reinsurance line.
  ...
  // Cancellation proportion is calculated from the endorsement detail:
  // previous coverage premium versus the negative coverage difference.
  const proportion = Math.min(coveragesDifference / oldCoverages, 1);
  ...
  const statements = cessions.map(cession => {
    const id = Number(cession.id);
    return `UPDATE Cession
SET premiumCedant = ROUND(ISNULL(premiumCedant, 0) * ${proportion}, 2),
    premiumRe = ROUND(ISNULL(premiumRe, 0) * ${proportion}, 2),
    comissionCedant = ROUND(ISNULL(comissionCedant, 0) * ${proportion}, 2),
    tax = ROUND(ISNULL(tax, 0) * ${proportion}, 2)
WHERE id = ${id}
  AND lifePolicyId = ${policyId}
  AND premiumType = 'CANCELLATION';`;
  });

  statements.push(`UPDATE CessionPart
SET premium = ROUND(ISNULL(premium, 0) * ${proportion}, 2),
    commission = ROUND(ISNULL(commission, 0) * ${proportion}, 2),
    tax = ROUND(ISNULL(tax, 0) * ${proportion}, 2)
WHERE cessionId IN (${cessionIds.join(",")});`);
