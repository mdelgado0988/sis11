...
/*
  *@name: cmdCalculatePremiumSuretyBond
...
try {
  ...
  // An order-of-proceeding endorsement only moves coverage dates. Keep the
  // current coverage values and skip tariff/object calculations completely.
  if (isProceedOrder) {
    setCurrentCoverageValues();
...
function setTarifas() {
  ...
  doCmd({cmd :"GetFullTable", data: {table: "tarificacionsis9Fianza"}});
...
function setCurrentCoverageValues() {
  for (let cov of (poliza.Coverages || [])) {
    const resultCoverage = resultCoverages.find(x => x.code == cov.code);
    if (!resultCoverage) continue;

    resultCoverage.limit = n(cov.limit !== undefined && cov.limit !== null ? cov.limit : cov.startLimit);
...
function getEndorsementType() {
  ...
  try {
    const changeData = extra && extra.data ? extra.data : null;
    jAdditional = (changeData && changeData.jAdditional)
      || (extra && extra.jAdditional)
      || "";

    if (!jAdditional && poliza.jChangeDto && poliza.jChangeDto.data) {
      jAdditional = poliza.jChangeDto.data.jAdditional || "";
    }

    if (!jAdditional) return "";
    const parsed = typeof jAdditional === "string" ? JSON.parse(jAdditional) : jAdditional;
    return String(parsed && parsed.endorsementType || "");
  } catch (errorType) {
    return "";
  }
}
...
poliza:
  ...
  IF ({DURACION_D}>= 365) BEGIN select Convert(decimal(10,4),{DURACION_D}/365.00) END ELSE BEGIN   select 1 END
  SELECT ISNULL(QDURACION,0) AS DURACION_D FROM ofpolcob WHERE ccober = {CCOBER} AND cproces = {CPROCES}
